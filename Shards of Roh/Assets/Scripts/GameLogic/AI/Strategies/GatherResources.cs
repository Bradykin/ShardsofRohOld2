...
public class GatherResources : Strategies {
	...
	public override void enact () {
		...
		if (interval >= 2 || AI.isNewBuilding == true) {
			interval = 0;
			AI.isNewBuilding = false;
			...
			if (GameManager.logging.gatherResources == true) {
				GameManager.print ("Resources needed: " + resourceNeeded + ", Villager Allocation: " + alloc.food + ", " + alloc.wood + ", " + alloc.gold + ", " + alloc.metal + ", " + alloc.build);
			}
	...
	private void processVillagerAllocation (Vector4 _resourceNeeded, float _neededBuild, float _availableVillagers) {
		...
		if (_neededBuild > 0 && _availableVillagers >= 4) {
			alloc.build += 2;
			_availableVillagers -= 2;
		}

		while (_availableVillagers > 0) {
			float needMost = Mathf.Max (_resourceNeeded.x, Mathf.Max (_resourceNeeded.y, Mathf.Max (_resourceNeeded.z, _resourceNeeded.w)));

			if (alloc.build < 3) {
				needMost = Mathf.Max (needMost, _neededBuild);
			}
